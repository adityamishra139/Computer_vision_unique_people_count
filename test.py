...
def build_faiss_indexes():
    global face_index, clothes_index, face_id_map, clothes_id_map
    face_vecs = []
    face_id_map = []
    cloth_vecs = []
    clothes_id_map = []
    for cid, data in daily_identity_db.items():
        for fv in data['face_features']:
            face_vecs.append(fv)
            face_id_map.append(cid)
        for cv in data['clothes_features']:
            cloth_vecs.append(cv)
            clothes_id_map.append(cid)
            
    if face_vecs:
        dim = len(face_vecs[0])
        face_index = faiss.IndexFlatIP(dim)
        face_index.add(np.array(face_vecs, dtype=np.float32))
    else:
        face_index = None
        
    if cloth_vecs:
        dim = len(cloth_vecs[0])
        clothes_index = faiss.IndexFlatIP(dim)
        clothes_index.add(np.array(cloth_vecs, dtype=np.float32))
    else:
        clothes_index = None
...
def tracking_worker():
    while True:
        ...
        # Optimize Tracker Speed (lag fix)
        body_results = body_model.track(raw_frame, classes=[0], conf=0.50, imgsz=256, persist=True, tracker="bytetrack.yaml", verbose=False)
        face_results = face_model.predict(raw_frame, conf=0.50, imgsz=256, verbose=False)
        ...
        for r in body_results:
            for box in r.boxes:
                tid_out = int(box.id[0]) if box.id is not None else None
                if tid_out is None: continue
                ...
                if body_crop.shape[0] > 0 and body_crop.shape[1] > 0:
                    ...
                    if has_face or valid_body:
                        bodies.append((tid_out, (bx1, by1, bx2, by2), best_face, body_crop.copy(), face_crop))
...
def determine_identity(body_crop, face_crop, tid):
    ...
    if face_vec is not None:
        ...
        if face_index is not None and face_index.ntotal > 0:
            query = np.array([face_vec], dtype=np.float32)
            distances, indices = face_index.search(query, k=1)
            
            if indices[0][0] != -1:
                # FlatIP returns similarity
                best_sim = distances[0][0]
                min_face_dist = 1.0 - best_sim
                best_face_id = face_id_map[indices[0][0]]
            ...
            for active_tid, tdata in active_tracks.items():
                if tdata.get('id') == best_face_id:
                    if active_tid != tid and tdata.get('frames_since_analysis', 0) < 30:
                        ...
                        # 🔥 KEY FIX: allow reuse if same person (close distance)
                        if dist < 100:
                            # Same person → allow
                            pass
                        else:
                            print(f"[FACE BLOCK] ID {best_face_id} already active elsewhere distance ({dist:.1f}px). Skipping.")
                            best_face_id = None
                            break
                    ...
                    old_cx, old_cy = tdata['center']
                    dist = np.sqrt((old_cx - curr_cx)**2 + (old_cy - curr_cy)**2)
                    if dist > 100:  # Loosened to 100 to allow re-attach within radius
        ...
        if best_face_id is not None:
            # FIX 3: ADD FACE RE-ATTACH FOR STRONG MATCH
            if min_face_dist < 0.15:
                if len(daily_identity_db[best_face_id]['face_features']) < 15:
                    daily_identity_db[best_face_id]['face_features'].append(face_vec)
                build_faiss_indexes()
                if tid in active_tracks: active_tracks[tid]['lock_frames'] = 30
                return best_face_id, "FORCE REATTACH (FACE)"
    ...
    if clothes_index is not None and clothes_index.ntotal > 0:
        query = np.array([track_features], dtype=np.float32)
        distances, indices = clothes_index.search(query, k=1)
        if indices[0][0] != -1:
            best_sim = distances[0][0]
            best_id = clothes_id_map[indices[0][0]]
...
while True:
    ...
    for (matched_tid, body_box, face_box, body_crop, face_crop) in detected_bodies:
    ...
    curr_time = time.time()
    active_tracks = {k: v for k, v in active_tracks.items() if curr_time - v['last_seen'] < 8.0}
